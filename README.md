...
- **Visual Polish:** Five switchable themes (Tokyo Night, Hack The Box, Matrix, Kanagawa, Ubuntu), JetBrainsMono Nerd Font.
...
3. **Finish Up:**
    Restart your terminal or run `. $PROFILE`. The installer will prompt you to choose a theme (Tokyo Night, Hack The Box, Matrix, Kanagawa, or Ubuntu). Switch anytime with `theme <name>`.
...
theme ubuntu    # Ubuntu — aubergine background, orange accents
```
...
- **Prompt:** `configs\starship.toml` — Tokyo Night prompt (or `starship-htb.toml`, `starship-matrix.toml`, `starship-kanagawa.toml`, `starship-ubuntu.toml`)
